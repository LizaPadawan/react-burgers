...
const OrderIngredients: FC = () => {
  ...
  const status =
    (order && order.status === "done" && "Выполнен") ||
    (order && order.status === "pending" && "Готовится") ||
    (order && order.status === "created" && "Создан");
  ...
  const ingredients = useMemo(() => {
    ...
    order?.ingredients.forEach((ingredientId) => {
      if (ingredientId !== null || ingredientId !== undefined) {
        const ingredientItem = ingredientsStore.find(
  ...
  const uniqueIngredients = useMemo(() => {
    ...
    return ingredients.filter(
      (ingredient) => !table[ingredient._id] && (table[ingredient._id] = 1)
    );
  }, [ingredients]);
        ...
        <div className={styles.details}>
          ...
          <p
            className={`text text_type_main-default ${
              order.status === "done" && styles.finish
            } mb-15`}
          >
            {status}
          </p>
