...
const IngredientCard: FC<TIngredient> = (props) => {
  const constructorElements: Array<TIngredient> =
    useAppSelector(constructorSelector);
  let count: number = constructorElements.filter(
  ...
  return (
    ...
    >
      <div className={burgerIngredientsStyles.burger_ingredient_content}>
        <img
          src={props.image}
          className={burgerIngredientsStyles.burger_ingredient_image}
        />
        ...
        <p
          className={
            "text text_type_digits-default m-1 " +
            burgerIngredientsStyles.burger_ingredient_price
          }
        >
          {props.price}
        ...
        <p
          className={
            ...
            burgerIngredientsStyles.burger_ingredient_text
          }
        >
          {props.name}
        </p>
...
const IngredientGroup: FC<TIngredientGroupProps> = (props) => {
  ...
  return (
    <>
      <div
        id={props.tab}
        className={burgerIngredientsStyles.ingredient_group_title}
      >
        <p className="text text_type_main-medium">{groupTitle}</p>
      </div>
      {data
        .filter((item) => item.type == groupName)
        .map((jtem) => (
          <IngredientCard key={jtem._id} {...jtem} />
        ))}
    </>
  );
};
...
function BurgerIngredients() {
  ...
  const min = (values: Array<number>) =>
    values.reduce((x, y) => Math.min(x, y));

  const scroll = () => {
    const scrolltop = document.getElementById("scrollbody")?.scrollTop;

    const tabDiffs = tabsArr.map((item) =>
      Math.abs(
        (scrolltop ? scrolltop : 0) -
          Number(document.getElementById(item)?.offsetTop)
      )
    );
  ...
  return (
    <>
      <div className={burgerIngredientsStyles.burger_ingredients_panel}>
        ...
        <div
          className={burgerIngredientsStyles.burger_ingredients_body}
          onScroll={scroll}
          id="scrollbody"
        >
